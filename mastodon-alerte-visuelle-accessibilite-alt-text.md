...
* **Les paliers (15px / 30px)** : définissent la largeur des bandes. Plus les valeurs sont petites, plus les hachures sont serrées.

### 3. La mise en page (le cadre)

L'utilisation de `box-sizing: border-box` est cruciale. Elle force la bordure à s'afficher vers l'intérieur de l'image. Sans cela, l'épaisseur de la bordure pourrait décaler les éléments voisins et briser la mise en page de votre instance.
...
| Couleurs | L'identité visuelle | `#000` (noir) et `#FFFF00` (jaune) pour un look "danger" classique. |

## Procédure d'installation

### Pour un usage personnel (utilisateur)

Si vous souhaitez voir ces alertes uniquement sur votre navigateur :

...
### A. Pourquoi j'ai mis en place cette page
  ...
  * **Gestion des hôtes inconnus :** Mon proxy inverse (NPM) agit comme un filtre intelligent. Lorsque vous tapez une adresse, NPM examine l'en-tête de la requête. S'il n'existe aucune règle de redirection (aucun "**hôte proxy**") correspondant au nom de domaine demandé, **j'ai configuré** NPM pour qu'il affiche cette page personnalisée.
...
### A. Les technologies clés que j'utilise
...
4.  **Coller le code** : Je copie le code HTML/CSS complet ci-dessous et le colle dans le champ de texte prévu.
5.  **Sauvegarde** : Je clique sur le bouton de sauvegarde pour rendre la page active immédiatement.

### B. Le Code HTML/CSS complet que j'utilise
...
### C. Comportement suite à l'implémentation

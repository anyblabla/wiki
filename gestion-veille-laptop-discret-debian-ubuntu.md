...
1.  J'édite le fichier de configuration de **GRUB** :

    ```bash
    sudo nano /etc/default/grub
    ```

2.  J'ajoute le paramètre `mem_sleep_default=deep` dans `GRUB_CMDLINE_LINUX_DEFAULT`.
...
3.  J'actualise **GRUB** et redémarre pour appliquer la modification :
...
1.  Je tente la **Suspension**.
2.  Au retour, j'utilise le **Terminal** pour afficher les messages du cycle de démarrage précédent relatifs à la veille :
    ```bash
...
1.  **Vérification de la taille :**
    ...
    free -h
    ```
2.  **Si j'utilise une Partition Swap :** Je récupère l'**UUID** de la partition (`sudo blkid`).
3.  **Si j'utilise un Fichier Swap (courant) :**
...
1.  J'édite de nouveau `/etc/default/grub`.

2.  J'ajoute les paramètres `resume=UUID=...` (pour une partition) ou `resume=PARTITION` et `resume_offset=OFFSET` (pour un fichier **Swap**) à la ligne `GRUB_CMDLINE_LINUX_DEFAULT`.
...
3.  J'actualise **GRUB** et je redémarre.
...
En suivant cette méthodologie, je parviens presque systématiquement à diagnostiquer et à résoudre les problèmes de veille sur la grande majorité des ordinateurs portables sous **Linux**.